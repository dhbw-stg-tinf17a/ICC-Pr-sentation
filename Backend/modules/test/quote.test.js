...
describe('quote module', () => {
  describe('getQuoteOfTheDay', () => {
    it('should return the quote of the day with the given category', async () => {
      ...
      await expect(quote.getQuoteOfTheDay('funny')).resolves.toStrictEqual({ quote: "You're only as good as your last haircut.", author: 'Fran Lebowitz' });

      // check conversion to API request (only in this test case)
      expect(axios.get).toHaveBeenCalledTimes(1);
      expect(axios.get).toHaveBeenCalledWith(quote.endpoint, { params: { category: 'funny' } });
    });
    ...
    it('should return the default quote if the rate limit is reached', async () => {
      ...
      await expect(quote.getQuoteOfTheDay()).resolves.toStrictEqual({ quote: 'Only a stupid man would request that many quotes.', author: 'Gunter' });
    });
    ...
    it('should throw an error if the API returns an error', async () => {
      ...
      await expect(quote.getQuoteOfTheDay()).rejects.toThrow(error);
    });
  });
});

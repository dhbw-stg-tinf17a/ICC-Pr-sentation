...
describe('vvs module', () => {
  describe('getConnection', () => {
    it('should return a connection from given coordinates to a given address', async () => {
      ...
      await expect(vvs.getConnection({ originCoordinates: { latitude: 48.77355689485371, longitude: 9.17095497616363 }, destinationAddress: 'Neckartalstraße, 70376 Stuttgart, Germany', arrival: '2020-03-23T22:00:00Z' })).resolves.toStrictEqual(connection);
    ...
    it('should return undefined if no connection is found', async () => {
      ...
      await expect(vvs.getConnection({ originCoordinates: { latitude: 48.77355689485371, longitude: 9.17095497616363 }, destinationAddress: 'Schlumpfhausen', arrival: '2020-03-23T22:00:00Z' })).resolves.toBeUndefined();
    });
    ...
    it('should throw an error if the API returns an error', async () => {
      ...
      await expect(vvs.getConnection({ originCoordinates: { latitude: 48.77355689485371, longitude: 9.17095497616363 }, destinationAddress: 'Schlumpfhausen II', arrival: '2020-03-23T22:00:00Z' })).rejects.toThrow('VVS API returned: DROELFISNOTANUMBER');
    });
  });
});
